	...
	public boolean processInteract(EntityPlayer player, EnumHand hand, ItemStack stack) {
		if (stack != null && stack.getItem() == Items.GOLDEN_APPLE && player.isSneaking()) {
			if (!this.world.isRemote) {
				EntityFancyBadger entityfancybadger = new EntityFancyBadger(this.world);
				entityfancybadger.setLocationAndAngles(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch);
				this.setDead();
				this.world.spawnEntity(entityfancybadger);
			}
		...
		else if (!this.world.isRemote && (this.getPassengers().isEmpty() || this.getControllingPassenger() == player) && !this.isChild() && !player.isSneaking()) {
